...
// Use the same origin if VITE_API_URL is not set
...
const MealPlanPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchProfileData = async () => {
      try {
        ...
        if (response.data.activity_level) {
          setActivityLevel(response.data.activity_level);
        }
        
        // Set initial dietary preferences from profile
        if (response.data.dietary_preferences) {
          setSelectedPreferences(response.data.dietary_preferences);
        }
      } catch (err) {
    ...
    try {
      ...
      if (response.data) {
        ...
        if (response.data.meal_times?.length > 0) {
          const firstDate = new Date(response.data.meal_times[0].date).toLocaleDateString('en-US', { weekday: 'long' });
          setExpandedDay(firstDate);
        }
      }
  ...
  // Group meal times by date
  const groupedMealTimes = useMemo<DayPlan[]>(() => {
    if (!mealPlan?.meal_times) return [];
    
    const grouped = mealPlan.meal_times.reduce((acc: { [key: string]: DayPlan }, meal: MealTime) => {
      ...
      acc[dayName].meals.push({
        ...
        time: meal.time,
        title: meal.name,
        description: `${meal.meal_items.map(item => item.name).join(', ')}`,
        nutrition: {
      ...
      // Update day totals
      acc[dayName].totals.calories += meal.calories;
      acc[dayName].totals.protein += meal.protein;
      acc[dayName].totals.carbs += meal.carbs;
      acc[dayName].totals.fats += meal.fats;
  ...
  // Calculate weekly averages for the nutrition summary
  const weeklyAverages = useMemo(() => {
    if (!mealPlan) return null;
    
    const totalDays = groupedMealTimes.length || 1;
    return {
      calories: mealPlan.calories_target,
      protein: mealPlan.protein_target,
      carbs: mealPlan.carbs_target,
      fats: mealPlan.fats_target,
      actualCalories: groupedMealTimes.reduce((sum: number, day: any) => sum + day.totals.calories, 0) / totalDays,
      actualProtein: groupedMealTimes.reduce((sum: number, day: any) => sum + day.totals.protein, 0) / totalDays,
      actualCarbs: groupedMealTimes.reduce((sum: number, day: any) => sum + day.totals.carbs, 0) / totalDays,
      actualFats: groupedMealTimes.reduce((sum: number, day: any) => sum + day.totals.fats, 0) / totalDays,
    };
  }, [mealPlan, groupedMealTimes]);

  // Add activity level selection to filters
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2">
              ...
              >
                <div className="mt-4 pt-4 border-t border-gray-200">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                      <h3 className="text-sm font-medium text-gray-900 mb-3">Activity Level</h3>
                      <select
                        value={activityLevel}
                    ...
                    {/* Calorie adjustment */}
                    <div>
                      <h3 className="text-sm font-medium text-gray-900 mb-3">Goal</h3>
                      <select
                  ...
                  <div className="mt-4 text-right">
                    <button
                      onClick={regenerateMealPlan}
                      disabled={loading}
                      className={`px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center ml-auto`}
                    >
                      {loading ? (
                        <>
            ...
            <div className="bg-white rounded-lg shadow overflow-hidden">
              {mealPlan ? (
                groupedMealTimes.map((dayPlan: DayPlan) => (
                ...
                <div className="p-8 text-center text-gray-500">
                  No meal plan generated yet. Use the filters above to generate a personalized meal plan.
                </div>
              )}
            </div>
          </div>
                  ...
                  <div>
                    <div className="flex justify-between mb-2">
                      ...
                      <span className="text-sm font-medium text-blue-600">
                        {Math.round(weeklyAverages.actualCalories)} / {weeklyAverages.calories} kcal
                      </span>
                    </div>
                    <div className="w-full bg-gray-200 rounded-full h-2.5">
                      <div 
                        className="bg-blue-600 h-2.5 rounded-full" 
                        style={{ width: `${Math.min(100, (weeklyAverages.actualCalories / weeklyAverages.calories) * 100)}%` }}
                      ></div>
                    </div>
                  </div>
                  ...
                  <div>
                    <div className="flex justify-between mb-2">
                      ...
                      <span className="text-sm font-medium text-green-600">
                        {Math.round(weeklyAverages.actualProtein)}g / {weeklyAverages.protein}g
                      </span>
                    </div>
                    <div className="w-full bg-gray-200 rounded-full h-2.5">
                      <div 
                        className="bg-green-600 h-2.5 rounded-full" 
                        style={{ width: `${Math.min(100, (weeklyAverages.actualProtein / weeklyAverages.protein) * 100)}%` }}
                      ></div>
                    </div>
                  </div>
                  ...
                  <div>
                    <div className="flex justify-between mb-2">
                      ...
                      <span className="text-sm font-medium text-yellow-600">
                        {Math.round(weeklyAverages.actualCarbs)}g / {weeklyAverages.carbs}g
                      </span>
                    </div>
                    <div className="w-full bg-gray-200 rounded-full h-2.5">
                      <div 
                        className="bg-yellow-600 h-2.5 rounded-full" 
                        style={{ width: `${Math.min(100, (weeklyAverages.actualCarbs / weeklyAverages.carbs) * 100)}%` }}
                      ></div>
                    </div>
                  </div>
                  ...
                  <div>
                    <div className="flex justify-between mb-2">
                      ...
                      <span className="text-sm font-medium text-red-600">
                        {Math.round(weeklyAverages.actualFats)}g / {weeklyAverages.fats}g
                      </span>
                    </div>
                    <div className="w-full bg-gray-200 rounded-full h-2.5">
                      <div 
                        className="bg-red-600 h-2.5 rounded-full" 
                        style={{ width: `${Math.min(100, (weeklyAverages.actualFats / weeklyAverages.fats) * 100)}%` }}
                      ></div>
                    </div>
                  </div>
                </div>
